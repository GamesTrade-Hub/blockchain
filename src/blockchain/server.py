...
@high_level_handler()
def ping():
    return json.dumps({"pong": "oui", "waitingTxs": blockchain.txs.__dict__()}), 200
...
def register_nodes():
    ...
    return (
        jsonify({"message": message, "total_nodes": blockchain.nodes.__str__()}),
        code,
    )
...
def new_block():
    ...
    response, code = blockchain.new_authority_block(
